    ...
    // Deploy App as Docker Container in Docker Deployment server
     stage('Deploy App as Docker Container in Docker Deployment server'){
        sshagent(['ubuntudeploymentserver']) {
         ...
         sh "ssh -o StrictHostKeyChecking=no ubuntu@172.31.43.211 docker run -d --name mavenwebappcontainer --network customebridgenetwork -p 8082:8080 686255940829.dkr.ecr.ap-south-1.amazonaws.com/maven-web-application:${buildNumber}"  
    }
     }
